...
async function buildCharacterCardsBundle() {
  ...
  const profiles = await fetchTableWithFallbacks('profiles', [
    q('id,nickname,vk_user_id', 'nickname.asc'),
  ]);
  const profileById = new Map((profiles ?? []).map((row) => [row.id, row]));

  if (rows.length === 0) {
    const characters = await fetchTableWithFallbacks('characters', [
  ...
  for (const row of rows) {
    ...
    cards.push({
      id: row.id,
      owner_id: row.owner_id,
      owner_nickname: row.owner_nickname ?? profileById.get(row.owner_id)?.nickname ?? null,
      owner_vk_user_id: row.owner_vk_user_id ?? profileById.get(row.owner_id)?.vk_user_id ?? null,
